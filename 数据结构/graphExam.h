...
void MatToList(MatGraph g, AdjGraph *&G) {			//将邻接矩阵 g 转换成邻接表 G
	int i, j;
	ArcNode *p;
...
void ListToMat(AdjGraph *G, MatGraph &g) {		//将邻接表 G 转换成邻接矩阵
	int i;
	ArcNode *p;
	for (i = 0; i < G->n; i++) {									//扫描所有懂得单链表
		p = G->adjlist[i].firstarc;									//p 指向第 i 个单链表的头结点
		while (p != NULL) {											//扫描第 i 个单链表
